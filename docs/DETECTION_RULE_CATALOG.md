...
Current machine-readable contract: `atdr_rule_catalog_v5.31.1`. The machine-readable catalog is authoritative when prose and code differ.
...
| `paloalto_malware_threat` | Palo Alto malware or C2 threat | `malware_c2` | 20-55 | High vendor-event confidence | `log_type`, `subtype`, `category` | Palo Alto `THREAT` row whose type is virus, wildfire-virus, or spyware, or whose category is command-and-control, backdoor, or botnet; vendor severity score plus 10. Replaces `paloalto_threat_log` for that row. | Signature false positives, sinkholed or security-research destinations. | Confirm the signature, affected host, and whether the session was blocked; ATDR does not verify infection independently. |
...
| `repeated_large_outbound` | Repeated large outbound transfers | `data_exfiltration_suspicion` | 20 | Low behavioral confidence | `bytes_sent`, `bytes`, `src_ip`, `dst_ip`, zone path, event time | `high_outbound_bytes` fired and the same source sent at least 3 above-threshold outbound transfers to the same destination within five minutes. | Chunked backups to one endpoint, cloud sync, large approved uploads. | Check the destination owner and transfer schedule; repetition corroborates volume, it does not prove theft. |
...
| `high_bytes_outlier` | High byte-count outlier | `unknown_anomaly` | 20 | Low | `bytes` | Total bytes exceed the versioned threshold and the same transfer was not already flagged by `high_outbound_bytes` (one transfer is scored once). | Normal large transfer. | Check direction, destination, app, and baseline before assigning an attack type. |
