...
| `ATDR-NET-019` / `repeated_large_outbound` | `high_outbound_bytes` fired and the same source sent at least 3 above-threshold outbound transfers to the same destination within 5 minutes | 5m source window | `data_exfiltration_suspicion`, T1048 | Repetition corroborates a volume anomaly; it does not establish theft, content, or authorization. |
