# v5.22 Exact Cumulative Commit Allowlist
...
through v5.22 detection-evidence and supervised-rebuild work. It does not
...
- the private PAN-OS source file, its local path, raw rows, addresses, or
  fingerprints;
- v5.21/v5.22 manifests, assisted packs, blind packs, predictions, or generated
...
1. compare the changed-path set exactly with these 46 paths;
...
5. confirm the staging area is empty until the owner gives separate exact-path
   approval; and
6. confirm no blind decision, private pack, or model artifact is included.
