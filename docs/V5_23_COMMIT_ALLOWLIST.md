# v5.23 Exact Cumulative Commit Allowlist
...
through v5.23 detection-evidence, supervised-rebuild, and live-source acceptance
work. It does not authorize a commit or push.
...
- the private PAN-OS source file, its local path, raw rows, addresses, or
  fingerprints;
- v5.21-v5.23 manifests, assisted packs, blind packs, predictions, or generated
  reports under `ml_baseline_reviews/`;
- locked temporal-final, rolling, external, private-future, or blind decisions;
...
1. compare the changed-path set exactly with these 55 paths;
...
5. confirm the staging area is empty until the owner gives separate exact-path
   approval; and
6. confirm no blind decision, private pack, generated report, or model artifact
   is included.
