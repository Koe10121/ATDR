...
V563_WORKSPACE_NAME = "supervised_qualification_expansion"
...
def _paths() -> dict[str, Path]:
    return v563._paths(v563.V563_OUTPUT_DIR)


def _validate_protocol(output_dir: Path) -> dict[str, Any]:
    return v563.validate_expansion_protocol(output_dir)


def _campaign_status(output_dir: Path) -> dict[str, Any]:
    return v563.get_public_v563_status(output_dir)


def _load_state(path: Path) -> dict[str, Any]:
    try:
        return v563._load_review_state(path.parent)
    except (OSError, ValueError, v563.V563ExpansionError) as exc:
        raise EvidenceReviewIntegrityError(
            "expansion_review_state_invalid",
            "The supplemental qualification review state failed integrity validation.",
        ) from exc


def _write_state(path: Path, state: dict[str, Any]) -> None:
    v563._atomic_write_json(path, state)
...
def _authorize_owner(batch: dict[str, Any], current_user: User) -> None:
    ...
    if owner_id is None:
        raise EvidenceReviewError(
            "expansion_review_batch_not_started",
            "Start this supplemental review batch before opening evidence.",
        )
    if int(owner_id) != current_user.id:
        raise EvidenceReviewError(
            "expansion_review_batch_owned_by_another_reviewer",
...
def _assert_revision(batch: dict[str, Any], expected_revision: int) -> None:
    if int(batch.get("revision") or 0) != expected_revision:
        raise EvidenceReviewError(
            "expansion_review_revision_conflict",
...
def _assert_open(batch: dict[str, Any]) -> None:
    if batch.get("closed_at"):
        raise EvidenceReviewError(
            "expansion_review_batch_closed",
            "This formally closed supplemental review batch is immutable.",
        )
...
def _validated_rows(
    ...
    try:
        ...
        v563._review_progress(paths["output_dir"])
        if state is not None:
            for batch in (state.get("batches") or {}).values():
                if batch.get("owner_user_id") is not None and batch.get(
                    ...
                    raise v563.V563ExpansionError(
                        "A review batch is not bound to the locked expansion."
                    )
    except (OSError, ValueError, v562.V562CampaignError, v563.V563ExpansionError) as exc:
        raise EvidenceReviewIntegrityError(
            "expansion_review_integrity_failed",
            "The protected supplemental qualification workspace failed integrity validation.",
        ) from exc
...
def _assert_batch(protocol: dict[str, Any], batch_id: str) -> None:
    if batch_id not in _defined_batch_ids(protocol):
        raise EvidenceReviewError(
            "expansion_review_batch_not_found",
            "The requested supplemental review batch does not exist.",
            status_code=404,
        )
...
def _status(
    ...
    if not all(path.is_file() for path in required):
        return {
            ...
            "message": "Prepare the private v5.63 expansion with the guarded CLI.",
            **_safety_projection(),
        }
    rows, protocol = _validated_rows(paths, state)
    progress = v563._review_progress(paths["output_dir"])
    ...
    return {
        ...
        "development_training_can_begin": False,
        "message": (
            "All supplemental batches are closed; external source evidence remains required."
            if all_closed
...
def get_expansion_review_status(current_user: User) -> dict[str, Any]:
    paths = _paths()
...
def start_expansion_review_batch(
    current_user: User,
    *,
    batch_id: str,
) -> dict[str, Any]:
    ...
    with _WORKSPACE_LOCK:
        ...
        if owner_id is not None and int(owner_id) != current_user.id:
            raise EvidenceReviewError(
                "expansion_review_batch_owned_by_another_reviewer",
                "This supplemental review batch is assigned to another reviewer.",
                status_code=403,
            )
        existing_reviewers = {
        ...
        if owner_id is None and existing_reviewers and existing_reviewers != {
            ...
            raise EvidenceReviewError(
                "expansion_existing_owner_unverified",
        ...
        return _operation(
            "expansion_review_batch_started",
            batch_id,
            batch,
            status,
            next_item=(
...
def _item(
    ...
    if (
        ...
        raise EvidenceReviewError(
            "expansion_review_row_out_of_range",
            "The requested supplemental review item does not exist in this batch.",
            status_code=404,
        )
    row = rows[row_index]
...
def list_expansion_review_items(
    current_user: User,
    *,
    batch_id: str,
    offset: int = 0,
...
def save_expansion_review_item(
    current_user: User,
    *,
    batch_id: str,
    row_index: int,
    ...
    if normalized_decision in {"suspicious", "malicious"} and not normalized_attack_type:
        raise EvidenceReviewError(
            "expansion_attack_type_required",
    ...
    with _WORKSPACE_LOCK:
        ...
        try:
            progress = v563._review_progress(paths["output_dir"])
        except (v562.V562CampaignError, v563.V563ExpansionError) as exc:
            v562._atomic_write_csv(paths["working"], original_rows)
            raise EvidenceReviewIntegrityError(
                "expansion_review_save_integrity_failed",
...
def close_expansion_review_batch(
    current_user: User,
    *,
    batch_id: str,
    expected_revision: int,
    ...
    with _WORKSPACE_LOCK:
        ...
        if not indexes or any(
            ...
            raise EvidenceReviewError(
                "expansion_review_batch_incomplete",
        ...
        if not batch_progress.get("complete") or batch_progress.get("invalid"):
            raise EvidenceReviewError(
                "expansion_review_batch_invalid",
