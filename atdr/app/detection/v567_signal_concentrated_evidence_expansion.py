...
V567_VERSION = "v5.67-signal-concentrated-evidence-expansion-v1"
V567_PROTOCOL_VERSION = "v5.67-append-only-evidence-protocol-v1"
V567_OUTPUT_DIR = PROJECT_ROOT / "ml_baseline_reviews" / "v5_67_evidence_expansion"
V567_LATEST = "v5_67_signal_concentrated_expansion_latest.json"
V567_PROTOCOL_LOCK = "v5_67_append_only_protocol.json"
V567_SEALED_PACK = "v5_67_signal_concentrated_review_pack.csv"
V567_WORKING_COPY = "v5_67_signal_concentrated_review_working.csv"
V567_REVIEW_STATE = "v5_67_signal_concentrated_review_state.json"
V567_EXCLUSION_MANIFEST = "v5_67_append_only_exclusion.json"
V567_SOURCE_CUSTODY = "v5_67_primary_source_custody.json"
V567_REPORT_PREFIX = "v5_67_signal_concentrated_evidence_expansion"

PREPARE_CONFIRMATION = "PREPARE_V567_SIGNAL_CONCENTRATED_EVIDENCE_EXPANSION"
TARGET_SIGNAL_ROWS = 500
BATCH_SIZE = 100
EXPECTED_COMBINED_ROWS = 1500
DEVELOPMENT_ROLES = v565.DEVELOPMENT_ROLES
FIXED_QUALIFICATION_GATES = dict(v565.FIXED_QUALIFICATION_GATES)

# Re-calibrated coverage-group weighting for round-robin selection.
#
# After genuinely, independently reviewing all 1,455 development-role rows
# across v5.62+v5.63+v5.65 (1,500 rows total, 45 sealed), the ACTUAL observed
# threat-positive rate per coverage_group is:
#   unknown_transport_context     33/235  14.0%
#   incomplete_transport_context  24/241  10.0%
#   high_activity_context         19/241   7.9%
#   boundary_context                2/173   1.2%
#   vendor_security_context          1/230   0.4%
#   routine_service_context         0/171   0.0%
#   web_transport_context           0/164   0.0%
# `vendor_security_context` was included in v5.65's high-yield set (weighted
# 3x, 97 rows sampled) on the strength of the original 1,000-row v5.62+v5.63
# sample, but produced zero new threat-positive rows in that 500-row round --
# it was not actually predictive. This tier drops it from the high-yield set
# and concentrates weighting on the three categories that together account
# for 76 of the 79 threat-positive rows found so far (96.2%). As before, this
# is a disclosed, category-level stratification decision informed only by
# the reviewer's own already-recorded independent judgments on this same
# source -- never by a model or rule prediction on the NEW candidate rows --
# and every present category still receives some representation. Every row
# is still selected without looking at its own prediction or rule verdict,
# and the human still labels every row blind, exactly as before.
...
COVERAGE_GROUP_ROUND_ROBIN_WEIGHT = 5


class V567ExpansionError(RuntimeError):
    pass
...
def _paths(output_dir: Path = V567_OUTPUT_DIR) -> dict[str, Path]:
    output_dir = Path(output_dir)
    return {
        "output_dir": output_dir,
        "latest": output_dir / V567_LATEST,
        "protocol": output_dir / V567_PROTOCOL_LOCK,
        "sealed": output_dir / V567_SEALED_PACK,
        "working": output_dir / V567_WORKING_COPY,
        "review_state": output_dir / V567_REVIEW_STATE,
        "exclusion": output_dir / V567_EXCLUSION_MANIFEST,
        "source_custody": output_dir / V567_SOURCE_CUSTODY,
    }
...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise V567ExpansionError(
            "A protected v5.67 record failed integrity validation."
        ) from exc
...
def _v565_boundary_snapshot(
    output_dir: Path = v565.V565_OUTPUT_DIR,
    *,
    ...
    v563_paths = v563._paths(v563_output_dir)
    v565_paths = v565._paths(output_dir)
    ...
    v565_sealed_rows, v565_sealed_columns = v562._read_csv(v565_paths["sealed"])
    ...
    v565._assert_extended_pack(v565_sealed_rows, v565_sealed_columns, sealed=True)
    combined_rows = len(v562_sealed_rows) + len(v563_sealed_rows) + len(v565_sealed_rows)
    if expected_combined_rows is not None and combined_rows != expected_combined_rows:
        raise V567ExpansionError(
            "The locked v562+v563+v565 row count no longer matches its expected boundary."
        )
    ...
    v565_tokens = {str(row.get("review_token") or "") for row in v565_sealed_rows}
    combined_role_counts: Counter[str] = Counter()
    for rows in (v562_sealed_rows, v563_sealed_rows, v565_sealed_rows):
        combined_role_counts.update(Counter(str(row.get("evidence_role") or "") for row in rows))
    return {
        "protocol": protocol,
        "combined_rows": combined_rows,
        "v562_review_tokens": v562_tokens,
        "v563_review_tokens": v563_tokens,
        "v565_review_tokens": v565_tokens,
        "combined_review_tokens": v562_tokens | v563_tokens | v565_tokens,
        "combined_role_counts": {
            role: int(combined_role_counts.get(role, 0)) for role in DEVELOPMENT_ROLES
        },
        "protocol_file_digest": v547._file_sha256(v565_paths["protocol"]),
        "sealed_file_digest": v547._file_sha256(v565_paths["sealed"]),
        "protected_digest": str(protocol.get("protected_digest") or ""),
        "source_digest": str(protocol.get("source_digest") or ""),
        "evaluation_rows": int(
            (protocol.get("v563_reference") or {}).get("future_evaluation_rows") or 0
        ),
        "decisions_openable_by_v567": False,
    }


def _v565_public_boundary(snapshot: dict[str, Any]) -> dict[str, Any]:
    return {
        "protocol_valid": True,
...
def _signal_projection(row: dict[str, Any], *, family: str) -> dict[str, Any]:
    ...
    if role not in v562.DEVELOPMENT_ROLE_NAMES:
        raise V567ExpansionError(
            "Signal-concentrated evidence must remain in a development-safe role."
        )
    base["review_token"] = v547._stable_hash(
        {
            "version": V567_VERSION,
            "family": family,
...
def select_signal_concentrated_review_candidates(
    rows: Iterable[dict[str, Any]],
    *,
    consumed_review_tokens: set[str],
    limit: int = TARGET_SIGNAL_ROWS,
    ...
    for row in rows:
        ...
        v565_token = str(v565._extended_projection(row, family=family).get("review_token") or "")
        if (
            v562_token in consumed_review_tokens
            or v563_token in consumed_review_tokens
            or v565_token in consumed_review_tokens
        ):
            excluded["already_selected_v562_v563_or_v565_evidence"] += 1
            seen_families.add(family)
            continue
        candidate = _signal_projection(row, family=family)
    ...
    minimum_required = min(TARGET_SIGNAL_ROWS, limit)
    selection_gate = bool(
    ...
    return selected, {
        ...
        "original_pack_overlap_count": int(
            excluded.get("already_selected_v562_v563_or_v565_evidence", 0)
        ),
...
def _assert_signal_pack(rows: list[dict[str, Any]], columns: list[str], *, sealed: bool) -> None:
    v562._assert_pack_contract(rows, columns, sealed=sealed)
    if "review_batch_id" not in columns:
        raise V567ExpansionError("The signal-concentrated pack has no review batch.")
    for row in rows:
        if str(row.get("evidence_role") or "") not in v562.DEVELOPMENT_ROLE_NAMES:
            raise V567ExpansionError(
                "The signal-concentrated pack contains non-development evidence."
            )
        batch_id = str(row.get("review_batch_id") or "")
        if not batch_id.startswith("batch-"):
            raise V567ExpansionError(
                "The signal-concentrated pack contains an invalid review batch."
            )
...
def _prepare_workspace(
    ...
    *,
    ...
    v565_snapshot: dict[str, Any],
    source_digest: str,
    primary_source_tokens: set[str],
    profile: dict[str, Any],
    roles: dict[str, Any],
    output_dir: Path = V567_OUTPUT_DIR,
    v565_output_dir: Path = v565.V565_OUTPUT_DIR,
    v563_output_dir: Path = v563.V563_OUTPUT_DIR,
    v562_output_dir: Path = v562.V562_OUTPUT_DIR,
) -> dict[str, Any]:
    if not rows:
        raise V567ExpansionError("No signal-concentrated rows were selected.")
    paths = _paths(output_dir)
    columns = list(rows[0])
    _assert_signal_pack(rows, columns, sealed=True)
    ...
    if any(present) and not all(present):
        raise V567ExpansionError(
            "An incomplete v5.67 protected workspace already exists."
        )

    exclusion = {
        "schema_version": V567_VERSION,
        "created_at": _now(),
        "consumed_families": sorted(consumed_families),
        "v567_selected_families": sorted(selected_families),
        "counts": {
            "consumed_families": len(consumed_families),
            "v567_selected_families": len(selected_families),
    ...
    protocol = {
        "schema_version": V567_PROTOCOL_VERSION,
        "campaign_version": V567_VERSION,
        "created_at": _now(),
        ...
        "append_only": True,
        "v565_reference": {
            "protocol_file_digest": v565_snapshot["protocol_file_digest"],
            "sealed_file_digest": v565_snapshot["sealed_file_digest"],
            "protected_digest": v565_snapshot["protected_digest"],
            "source_digest": v565_snapshot["source_digest"],
            "combined_rows": int(v565_snapshot["combined_rows"]),
            "future_evaluation_rows": int(v565_snapshot["evaluation_rows"]),
            "decisions_accessed": False,
        },
        "signal_selected_rows": len(rows),
        "total_comparable_capacity": int(v565_snapshot["combined_rows"]) + len(rows),
        ...
        "partition": {
            ...
            "signal_development_roles_only": True,
            "v562_v563_v565_future_evaluation_labels_sealed": True,
        },
        "evidence": {
    ...
    if created:
        ...
        existing = validate_signal_protocol(
            output_dir,
            v565_output_dir=v565_output_dir,
            v563_output_dir=v563_output_dir,
            v562_output_dir=v562_output_dir,
            expected_combined_rows=int(v565_snapshot["combined_rows"]),
        )
        if not (
            existing.get("protected_digest") == protected_digest
            and existing.get("source_digest") == source_digest
            and int(existing.get("signal_selected_rows") or 0) == len(rows)
        ):
            raise V567ExpansionError(
                "A different immutable v5.67 expansion already exists."
            )
        protocol = existing
    return {
        "created": created,
...
def validate_signal_protocol(
    ...
    *,
    ...
    if not all(path.is_file() for path in required):
        raise V567ExpansionError("The v5.67 expansion is not prepared.")
    ...
    _assert_signal_pack(sealed_rows, sealed_columns, sealed=True)
    _assert_signal_pack(working_rows, working_columns, sealed=False)
    combined_rows = int((protocol.get("v565_reference") or {}).get("combined_rows") or 0)
    snapshot = _v565_boundary_snapshot(
        v565_output_dir,
        v563_output_dir=v563_output_dir,
        v562_output_dir=v562_output_dir,
    ...
    reference = protocol.get("v565_reference") or {}
    exclusion = _read_json(paths["exclusion"])
    custody = _read_json(paths["source_custody"])
    definitions = _batch_definitions(sealed_rows)
    if not (
        protocol.get("schema_version") == V567_PROTOCOL_VERSION
        and protocol.get("campaign_version") == V567_VERSION
        ...
        == int(protocol.get("signal_selected_rows") or 0)
        ...
        and (protocol.get("partition") or {}).get(
            "v562_v563_v565_future_evaluation_labels_sealed"
        )
        is True
        ...
        raise V567ExpansionError(
            "The immutable v5.67 expansion failed integrity validation."
        )
    return protocol


def _review_progress(output_dir: Path = V567_OUTPUT_DIR) -> dict[str, Any]:
    paths = _paths(output_dir)
    ...
    _assert_signal_pack(rows, columns, sealed=False)
    reviewed = 0
    invalid = 0
...
def _combined_gates(
    *,
    ...
    reviewed = (
        valid_count(v562_review)
        + valid_count(v563_review)
        + valid_count(v565_review)
        + valid_count(v567_review)
    )
    supports = [
        ...
        v567_review.get("development_class_support") or {},
    ]
...
def _load_review_state(output_dir: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        return {"schema_version": V567_VERSION, "batches": {}}
    state = _read_json(path)
    if state.get("schema_version") != V567_VERSION or not isinstance(
        state.get("batches"), dict
    ):
        raise V567ExpansionError("The v5.67 review state is invalid.")
    return state
...
def _public_protocol(protocol: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "valid": True,
        "append_only": True,
        "combined_prior_rows": int((protocol.get("v565_reference") or {}).get("combined_rows") or 0),
        "signal_rows": int(protocol.get("signal_selected_rows") or 0),
...
def get_public_v567_status(
    ...
    *,
    ...
    boundary_snapshot = _v565_boundary_snapshot(
        v565_output_dir,
        v563_output_dir=v563_output_dir,
        v562_output_dir=v562_output_dir,
        expected_combined_rows=None,
    )
    v562_review = v562._review_progress(v562_output_dir)
    v563_review = v563._review_progress(v563_output_dir)
    v565_review = v565._review_progress(v565_output_dir)
    paths = _paths(output_dir)
    if not paths["protocol"].is_file():
        v567_review = _review_progress(output_dir)
        return {
            "version": V567_VERSION,
            "status": "not_prepared",
            "v565_boundary": _v565_public_boundary(boundary_snapshot),
            ...
            "qualification_gates": _combined_gates(
                ...
                v567_review=v567_review,
            ),
            "development_training_can_begin": False,
            **_safety_projection(),
        }
    protocol = validate_signal_protocol(
        output_dir,
        v565_output_dir=v565_output_dir,
        v563_output_dir=v563_output_dir,
        v562_output_dir=v562_output_dir,
    ...
    batch_progress = v567_review.get("batch_progress") or {}
    batches = []
    closed_count = 0
    ...
    combined_reviewed = (
        ...
        + int(v563_review.get("reviewed") or 0)
        + int(v565_review.get("reviewed") or 0)
        + int(v567_review.get("reviewed") or 0)
    )
    combined_total = int(protocol.get("total_comparable_capacity") or 0)
    status = (
        "signal_review_closed"
        if batches and closed_count == len(batches)
        else "signal_review_in_progress"
        if v567_review.get("reviewed")
        else "ready_for_signal_review"
    )
    ...
    return {
        ...
        "evidence": {
            ...
            "signal_rows_selected": int(protocol.get("signal_selected_rows") or 0),
        ...
        "development_blockers": [
            "complete_and_close_all_v562_v563_v565_v567_review",
...
def load_reviewed_combined_development_rows(
    ...
    *,
    ...
    """Load reviewed development rows across v562+v563+v565+v567; evaluation rows stay unreachable."""
    ...
    batch_ids = {
        ...
        for item in validate_signal_protocol(
            output_dir,
            v565_output_dir=v565_output_dir,
            v563_output_dir=v563_output_dir,
            v562_output_dir=v562_output_dir,
    ...
    if not batch_ids or any(
        ...
        raise V567ExpansionError(
            "Signal-concentrated development labels remain sealed until every batch closes."
        )
    signal_rows, _ = v562._read_csv(_paths(output_dir)["working"])
    if not _review_progress(output_dir).get("complete"):
        raise V567ExpansionError("Signal-concentrated development labels remain incomplete.")
    if any(
        str(row.get("evidence_role") or "") == v562.EVALUATION_ROLE_NAME
        for row in signal_rows
    ):
        raise V567ExpansionError("Evaluation evidence entered development code.")
    combined = v565.load_reviewed_combined_development_rows(
        v565_output_dir, v563_output_dir=v563_output_dir, v562_output_dir=v562_output_dir
    )
    return [*combined, *signal_rows]
...
def _safe_failure(status: str, *, stage: str, error_type: str) -> dict[str, Any]:
    return {
        ...
        "message": "The v5.67 operation failed closed without changing governed state.",
        **_safety_projection(),
    }
...
def _render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# v5.67 Signal-Concentrated Evidence Expansion",
            "",
            f"- Status: `{result.get('status')}`",
            f"- Combined prior rows preserved: `{evidence.get('combined_prior_rows_preserved', 0)}`",
            f"- Signal-concentrated rows selected: `{selection.get('selected_rows', 0)}`",
...
def run_v567_signal_concentrated_evidence_expansion(
    db: Session,
    *,
    sample_path: Path | None,
    use_temp_db: bool = False,
    preflight_only: bool = True,
    prepare_signal_review: bool = False,
    confirmation: str | None = None,
    signal_limit: int = TARGET_SIGNAL_ROWS,
    batch_size: int = BATCH_SIZE,
    output_dir: Path = V567_OUTPUT_DIR,
    v565_output_dir: Path = v565.V565_OUTPUT_DIR,
    v563_output_dir: Path = v563.V563_OUTPUT_DIR,
    ...
    stage = "v565_boundary_validation"
    try:
        boundary = _v565_boundary_snapshot(
            v565_output_dir,
            v563_output_dir=v563_output_dir,
            v562_output_dir=v562_output_dir,
        ...
        return _safe_failure(
            "failed_closed_v565_boundary", stage=stage, error_type=exc.__class__.__name__
        )
    available = bool(sample_path and Path(sample_path).is_file())
    if preflight_only and not prepare_signal_review:
        ...
        return {
            ...
            "preflight_only": True,
            "v565_boundary": _v565_public_boundary(boundary),
            "private_file": {
    ...
    if int(signal_limit) < TARGET_SIGNAL_ROWS:
        return _safe_failure(
            "signal_capacity_below_fixed_target",
            stage="capacity_gate",
    ...
    try:
        ...
        if sample_digest != boundary["source_digest"]:
            raise V567ExpansionError(
                "The supplied source does not match the immutable v562/v563/v565 source."
            )
        with tempfile.TemporaryDirectory(prefix="atdr-v567-") as directory:
            ...
            try:
                ...
                if not profile.get("ok"):
                    raise V567ExpansionError("Private evidence parsing failed.")
                ...
                v56.build_disposable_behavior_aggregates(connection)
                # v562's, v563's, and v565's own tokens were each computed only
                # after the prior tier's families were quarantined and behavior
                # aggregates were rebuilt. Reconstructing all three exactly
                # requires replaying that same three-phase quarantine-then-
                # reaggregate sequence in order, not a single flat pass.
                ...
                connection.execute(
                    "CREATE TEMP TABLE v567_v562_families (family TEXT PRIMARY KEY)"
                )
                connection.executemany(
                    "INSERT INTO v567_v562_families(family) VALUES (?)",
                ...
                connection.execute(
                    ...
                    "(SELECT family FROM v567_v562_families)"
                )
                connection.commit()
                v56.build_disposable_behavior_aggregates(connection)
                ...
                connection.execute(
                    "CREATE TEMP TABLE v567_v563_families (family TEXT PRIMARY KEY)"
                )
                connection.executemany(
                    "INSERT INTO v567_v563_families(family) VALUES (?)",
                    [(family,) for family in sorted(consumed_families)],
                )
                connection.execute(
                    "UPDATE events SET role_rank=4, "
                    ...
                    "WHERE candidate_near_hash IN "
                    "(SELECT family FROM v567_v563_families)"
                )
                connection.commit()
                v56.build_disposable_behavior_aggregates(connection)

                stage = "v565_append_only_exclusion"
                representatives = v547._load_representatives(connection)
                v565_matched_tokens: set[str] = set()
                for row in representatives:
                    family = str(row.get("_candidate_family") or "")
                    if not family:
                        continue
                    ...
                    if role not in v562.DEVELOPMENT_ROLE_NAMES:
                        continue
                    v565_token = str(
                        v565._extended_projection(row, family=family).get("review_token") or ""
                    )
                    if v565_token in boundary["v565_review_tokens"]:
                        consumed_families.add(family)
                        v565_matched_tokens.add(v565_token)
                if v565_matched_tokens != boundary["v565_review_tokens"]:
                    raise V567ExpansionError(
                        "The locked v565 evidence could not be reconstructed exactly."
                    )
                connection.execute(
                    "CREATE TEMP TABLE v567_consumed_families (family TEXT PRIMARY KEY)"
                )
                connection.executemany(
                    "INSERT INTO v567_consumed_families(family) VALUES (?)",
                    [(family,) for family in sorted(consumed_families)],
                )
                connection.execute(
                    "UPDATE events SET role_rank=4, "
                    "quarantine_reason='already_selected_v562_v563_or_v565_evidence' "
                    "WHERE candidate_near_hash IN "
                    "(SELECT family FROM v567_consumed_families)"
                )
                connection.commit()
                v56.build_disposable_behavior_aggregates(connection)
                stage = "signal_concentrated_prediction_blind_selection"
                representatives = v547._load_representatives(connection)
                candidates, selection, selected_families = select_signal_concentrated_review_candidates(
                    representatives,
                ...
                stage = "v565_mutation_guard"
                after_guard = _v565_boundary_snapshot(
                    v565_output_dir,
                    v563_output_dir=v563_output_dir,
                    v562_output_dir=v562_output_dir,
                    expected_combined_rows=EXPECTED_COMBINED_ROWS,
                ...
                if before_guard != {
                    ...
                    raise V567ExpansionError(
                        "The locked v562+v563+v565 protected pack changed during preparation."
                    )
    ...
    public_status = get_public_v567_status(
        output_dir,
        v565_output_dir=v565_output_dir,
        v563_output_dir=v563_output_dir,
        v562_output_dir=v562_output_dir,
    ...
    evidence = {
        ...
        "signal_rows_selected": int(selection.get("selected_rows") or 0),
        "total_comparable_capacity": int(protocol.get("total_comparable_capacity") or 0),
        "real_source_identities": len(primary_source_tokens),
        "independent_time_windows": int(roles.get("distinct_time_windows") or 0),
        "second_source_present": False,
    ...
    if write_report:
        ...
        (output_dir / f"{V567_REPORT_PREFIX}_{_stamp()}.md").write_text(
