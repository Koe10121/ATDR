...
def test_react_frontend_docs_and_cors_are_wired():
    ...
    dashboard_path = Path("docs/archive/legacy/DASHBOARD_PRODUCTION_PATH.md").read_text(
        encoding="utf-8"
    )
...
def test_real_data_ai_demo_helper_is_documented_and_private_logs_are_ignored():
    ...
    current_runbook = Path("docs/AI_TRAINING_RUNBOOK.md").read_text(encoding="utf-8")
    historical_runbook = Path(
        "docs/archive/runbooks/AI_TRAINING_RUNBOOK_THROUGH_V5_58.md"
    ...
    assert "Label Integrity" in current_runbook
    assert "Real Data Demo Checklist" in historical_runbook
    assert "run_ai_demo_pipeline.ps1" in historical_runbook
...
def test_assisted_labeling_scripts_and_runbook_warning_exist():
    ...
    assert "assisted suggestions remain weak labels" in runbook
    ...
    assert "Human-reviewed means" in runbook
