...
// The worklist is sorted by priority, so the first rows are the ones to label next.
const QUEUE_PREVIEW_ROWS = 5;
...
export function MLGovernance() {
  ...
  const frozenEvaluation = useFrozenEvaluationStatus();
  ...
  return (
    <div className="space-y-5">
      ...
      <AiTrustSummary
        runtime={detectionRuntime.data}
        report={data}
        assistant={assistantStatus.data}
        evaluation={frozenEvaluation.data}
        bootstrapCommand={anomalyBootstrapCommand}
      />
        ...
        {reviewQueue.isLoading ? (
          ...
          <div className="overflow-auto">
            <table className="soc-table soc-table-compact">
              ...
              <tbody>
                {(showFullQueue ? reviewQueue.data : reviewQueue.data.slice(0, QUEUE_PREVIEW_ROWS)).map((item) => (
                  <tr key={item.log_id}>
                    <td>
            ...
            {reviewQueue.data.length > QUEUE_PREVIEW_ROWS ? (
              <button className="btn-secondary mt-3" type="button" onClick={() => setShowFullQueue((value) => !value)}>
                {showFullQueue ? `Show the top ${QUEUE_PREVIEW_ROWS} only` : `Show all ${reviewQueue.data.length} logs in the worklist`}
              </button>
            ) : null}
          </div>
        ) : (
